...
def get_node_center(key: str) -> tuple[int, int]:
    n = NODES[key]
...
def render_frame(step_idx: int, subframe_idx: int, total_subframes: int) -> Image.Image:
    step = STEPS[step_idx]
    ...
    draw.text(
        (25, 43),
        "Autonomous Pipeline Observability & Self-Healing Execution Flow",
    ...
    if t >= 0.7:
        ...
        draw.ellipse(
            ...
            outline=(bc[0], bc[1], bc[2], ripple_alpha),
            width=2,
        )
    ...
    for k, n in NODES.items():
        ...
        if is_active:
            ...
            draw.rounded_rectangle(
                [(nx - halo_pad, ny - halo_pad), (nx + nw + halo_pad, ny + nh + halo_pad)],
                radius=12,
                fill=None,
                outline=(c[0], c[1], c[2], 100 if is_focused else 50),
                width=2,
            )
        ...
        draw.rounded_rectangle(
            [(nx, ny), (nx + nw, ny + nh)], radius=10, fill=fill_bg, outline=outline_col, width=border_w
        )

        # Left colored accent bar
        accent_alpha = 255 if is_active else 120
        draw.rounded_rectangle(
            [(nx + 3, ny + 10), (nx + 7, ny + nh - 10)], radius=2, fill=(c[0], c[1], c[2], accent_alpha)
        )
        ...
        draw.rounded_rectangle(
            ...
            radius=5,
            fill=(20, 28, 44, 255),
            outline=(c[0], c[1], c[2], 160 if is_active else 60),
            width=1,
        )
    ...
    draw.text(
        (term_x + 72, term_y + 8),
        "DATAGUARD SENTINEL REAL-TIME AUDIT & TRACE LOG",
        fill=(148, 163, 184, 255),
        font=font_term_head,
    )
    ...
    draw.rounded_rectangle(
        [(term_x + term_w - stag_w - 15, term_y + 6), (term_x + term_w - 15, term_y + 24)],
        radius=5,
        fill=(24, 34, 52, 255),
        outline=step["color"],
        width=1,
    ...
    draw.text(
        (term_x + 18, term_y + 116), f"AUDIT IMPACT: {step['detail']}", fill=(148, 163, 184, 255), font=font_node_sub
    )
    draw.text(
        (term_x + 18, term_y + 134),
        "ARCHITECTURE: FastMCP Router -> Distributed State Engine -> PostgreSQL System of Record",
...
def main() -> None:
    ...
    frames[0].save(output_path, save_all=True, append_images=frames[1:], duration=130, loop=0, optimize=True)
